...
- **state** (resume): `{ version:1, mode, n, regions, solution, cells, elapsedMs, solved }`.
- **stats**: `{ easy|medium|hard|veryhard: {bestMs, wins}, custom: { [nAsString]: {bestMs, wins} } }`.
- **settings**: `{ theme, palette, autoX, highlightConflicts, showTimer, defaultMode, customN }`.
...
js/
  rng.js modes.js generator.js solver.js puzzle.js rules.js colors.js
