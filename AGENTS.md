...
- `js/history.js` — immutable solved-puzzle history (`recordSolve` prepends + caps at 50;
  `normalizeHistory`/`normalizeEntry` validate defensively).
...
### Side-effecting / glue
- `js/storage.js` — defensive `localStorage` wrapper (stats, settings, resume, history). Accepts an
...
js/
  ...
  serialize.js stats.js history.js settings.js game.js     # pure / testable
